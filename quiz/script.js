...
function concludeExamination() {
    stopTimer();
    if (window.saveScoreToDatabase) window.saveScoreToDatabase(AppState.score);
...
// FLOATING BACK BUTTON BINDING:
// Conditional navigation based on current view screen
DOM.controls.globalBack.addEventListener('click', () => {
    // Check if the welcome screen is currently the active view
    const isAtWelcomeScreen = DOM.views.welcome.classList.contains('active-screen');

    if (isAtWelcomeScreen) {
        // Absolute path routing to Gallery when on Home/Welcome screen
        window.location.href = '../gallery.html';
    } else {
        // Standard interactive UI back behavior: stop active timers and return home
        stopTimer();
